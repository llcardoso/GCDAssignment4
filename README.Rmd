...
The purpose of this file is to give comprehensive comments on getting, cleaning and processing the data for this exercise from the Getting and Cleaning Data Course from Johns Hopkins University. For that reason, the presented code is not delimited into code-chunks. You may download the [UCI HAR Dataset](http://archive.ics.uci.edu/dataset/240/human+activity+recognition+using+smartphones) and run and reproduce my analysis directly by running the code on the "run_analysis.R" file.
...
**To name the measurements, I am using the authors' expressions on the features.txt file. I am creating a vector with the names, which I can then add as column names:**
...
**Now we have the vectors corresponding to the columns in which the means and the stds for each measurement are stated, for the concatenated dataset of training and test data. What is left for point 2 is to subset the data according to these vectors and joining the means and stds in a single dataset:**
...
**As I am using the authors' feature file as column name giver, I am assuming these as appropriate column names. To complete points 3 and 4, now we have to label the data by activity and subject. To keep the data tidy, the best option is to use the labels given by the authors, convert them into an appropriate label (like the name of the activity or subject number) and add  this information as additional columns in the data set.**
...
**Thankfully, the authors made it so that the subject ID has a unique key across test and training datasets. As such, the labels could be safely merged directly. As subject names are confidential, I will keep the numeric key as their identifier.**
...
**So, now we have identifiers for both the individuals and the activity. All we need to do now is to merge these IDs with the data from the X files.**
...
Now for step 5, we need to average the means by subject and activity. For this purpose, I am going to use lapply on the columns carrying "mean" or "std" (excluding the label columns I added) to turn them into numeric, and then I am doing a new dataframe consisting of the averages of each Activity and Subject combination.**
...
write.table(averages_mean_std, file = "Exercise.txt",row.name=FALSE) 
